...
Behaviour stabilised alongside them:
...
- `FA102` checks more PEP 585-compatible APIs, including
  `collections.abc`.
- `INT001`/`INT002`/`INT003` recognise more `gettext` idioms, including
...
- `UP019` recognises `typing_extensions.Text`.

## Stabilised in 0.15.0
...
Enable with `preview = true`; expect them to stabilise in a later
release.
...
- Python 3.14 is the default and maximum target from 0.14.0.
...
- `frozendict` is recognised as a builtin on 3.15+ (0.15.8).
